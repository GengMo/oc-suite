...
/** _DynamicDelegate implements a class's delegate, data source, or other
 delegated protocol by associating protocol methods with a block implementation.
	...
	- (IBAction) annoyUser
	{
		...
		_DynamicDelegate *dd = alertView.bk_dynamicDelegate;
 ...
 _DynamicDelegate is designed to be 'plug and play'.
 */


@interface _DynamicDelegate : NSProxy

/**
 * The designated initializer for the _DynamicDelegate proxy.
 *
 * An instance of _DynamicDelegate should generally not be created by the user,
 * but instead by calling a method in NSObject(_DynamicDelegate). Since
 ...
 * ends. NSObject(_DynamicDelegate) creates a strong reference.
 *
...
/** A dictionary of custom handlers to be used by custom responders
 in a A2Dynamic(Protocol Name) subclass of _DynamicDelegate, like
...
/**
 * @desc Block Instance Method Implementations 
 */
- (nullable id)implFor:(SEL)selector;
...
/** Assigns the given block to be fired when the specified
 ...
 @warning Starting with _DynamicDelegate 2.0, a block will
 ...
 */
- (void)impl:(SEL)selector with:(nullable id)block;
...
/** Disassociates any block so that nothing will be fired
 ...
 @param selector An encoded selector. Must not be NULL.
 */
- (void)removeImplFor:(SEL)selector;
...
/** Assigns the given block to be fired when the specified
 ...
 @param block A code block with the same signature as selector.
 */
- (void)impls:(SEL)selector with:(nullable id)block;
...
/** Disassociates any blocks so that nothing will be fired
 ...
 @param selector An encoded selector. Must not be NULL.
 */
- (void)removeImplsFor:(SEL)selector;
