    ...
    [[UMSocialManager defaultManager]
     ...
     completion:^(id result, NSError *error) {
         ...
         if (error) {
             ...
             [respDict safeSetObject:resp.uid forKey:@"uid"];
             [respDict safeSetObject:resp.openid forKey:@"openid"];
             [respDict safeSetObject:resp.accessToken forKey:@"accessToken"];
             [respDict safeSetObject:resp.refreshToken forKey:@"refreshToken"];
             [respDict safeSetObject:resp.expiration forKey:@"expiration"];
             [respDict safeSetObject:resp.name forKey:@"name"];
             [respDict safeSetObject:resp.iconurl forKey:@"iconurl"];
             [respDict safeSetObject:resp.unionGender forKey:@"unionGender"];
             
             invoke_nullable_block(successHandler, respDict)
         }
    }];
}
