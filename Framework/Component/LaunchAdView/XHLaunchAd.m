 ...
 *  未检测到广告数据,启动页默认停留时间
 */
static NSInteger const noDataDefaultDuration = 2;
...
@property (nonatomic, assign) LaunchAdSkipType skipType;
...
- (void)setImageUrl:(NSString *)imageUrl
           ...
           skipType:(LaunchAdSkipType)skipType
            options:(XHWebImageOptions)options
          completed:(XHWebImageCompletionBlock)completedBlock
...
- (UIButton *)skipButton {
    if(_skipButton == nil) {
        ...
        [_skipButton setEnlargeEdgeWithTop:10 left:5 bottom:10 right:5];
        ...
        if(!_skipType) _skipType = LaunchAdSkipTypeTimeText;//类型传nil,默认TimeText
...
- (UIImage *)getLaunchImage {
    ...
    exceptioning(@"获取LaunchImage失败!请检查是否添加启动图,或者规格是否有误.")
