...
- (id)initWithframe:(CGRect)frame Delegate:(id<UUDatePickerDelegate>)delegate PickerStyle:(DatePickerStyle)uuDateStyle {
    self.datePickerStyle = uuDateStyle;
    self.delegate = delegate;
    return [self initWithFrame:frame];
}

- (id)initWithframe:(CGRect)frame PickerStyle:(DatePickerStyle)uuDateStyle didSelected:(FinishBlock)finishBlock {
    self.datePickerStyle = uuDateStyle;
...
- (void)drawRect:(CGRect)rect {
    if (self.frame.size.height<keyboard_height || self.frame.size.width<screen_width)
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, screen_width, keyboard_height);
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
         ...
         */
        if (IOS7_OR_LATER) {
            [self creatValuePointXs:@[[NSString stringWithFormat:@"%.0f", screen_width/2-100/2],
                                      [NSString stringWithFormat:@"%.0f", screen_width/2+100/2 - 20],
                                      [NSString stringWithFormat:@"%.0f", screen_width/2+100/2+50]]
                      withNames:@[@"年",@"月",@"日"]];
            }
        return 3;
    }
...
- (void)playTheDelegate {
    // 在该方法前，进行了reloadAllComponents，此时只更新了picker组件的数据部分，还没有绘图；如果在没绘图的时候，更新了datasource、却没reload，则会出现错位。所以在外部，加上的队列，将它排到main thread的后头去，保证时序OK！！！
...
- (NSInteger)DaysfromYear:(NSInteger)year andMonth:(NSInteger)month
{
    ...
    LOG(@"month_index = %d", (int)num_month);
