...
@protocol AnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(AnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(AnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(AnimatedImage *)animatedImage;
@end
#endif
...
// The actual type of the object is `WeakProxy`.
@property (nonatomic, strong, readonly) AnimatedImage *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<AnimatedImageDebugDelegate> debug_delegate;
#endif
...
- (instancetype)init
{
    AnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
...
{
    AnimatedImage *animatedImage = [[AnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
@implementation AnimatedImage (Logging)
...
// This is the designated creation method of an `WeakProxy` and
